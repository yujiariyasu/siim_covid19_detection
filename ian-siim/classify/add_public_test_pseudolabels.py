...
SAVE_DIR = '../pseudosegs/public_test_avg'
if not osp.exists(SAVE_DIR): os.makedirs(SAVE_DIR)

pseudosegs = glob.glob('../pseudosegs/public_test_0/*')
for ps in tqdm(pseudosegs):
    seglist = []
    for fold in range(5):
        seglist.append(cv2.imread(ps.replace('public_test_0', f'public_test_{fold}'), 0).astype('float32'))
    seglist = np.asarray(seglist)
    seglist = np.mean(seglist, axis=0)
    seglist = seglist.astype('uint8')
    new_fp = ps.replace('public_test_0', 'public_test_avg')
    status = cv2.imwrite(new_fp, seglist)


# Load in classification predictions for each fold and average
pseudocls = glob.glob('../predictions/public_test*csv')
...
pseudocls['filename'] = pseudocls.imgfile.apply(lambda x: x.replace('../data/covid/', ''))
pseudocls['outer'] = -1
pseudocls['boxes'] = pseudocls.filename.apply(lambda x: osp.join('../pseudosegs/public_test_avg', x.split('/')[-1]))

df['filename'] = df.filename.apply(lambda x: osp.join('train_pngs', x))
df = pd.concat([df, pseudocls])
df.to_csv('../data/covid/train_kfold_cleaned_w_bboxes_plus_public_test_pseudo.csv', index=False)
