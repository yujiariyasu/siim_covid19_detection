...
def make_batch_script(cfg, gpu, kfold, single_node):
    prefix = cfg.split('/')[-1].replace('.yaml', '')
    folder = cfg.split('/')[-2]
    name = f'{prefix}_{gpu}'
    with open(f'batch-scripts/{name}.sh', 'w') as f:
        ...
        f.write('cd /blue/derek.merck/ianpan/cov2/classify/\n')
        srunline = f'srun /home/ianpan/anaconda3/envs/pl/bin/python main.py train configs/{folder}/{prefix}.yaml --benchmark --precision 16 --gpus {4 if single_node else 1} --num-workers 4 --sync_batchnorm --num_nodes {1 if single_node else 4} --accelerator ddp'
        if kfold > 0:
