...
bbox_df['int_label'] = np.argmax(bbox_df[['negative','indeterminate','atypical','typical']].values, axis=1)
...
for i in tqdm(range(len(bbox_df)), total=len(bbox_df)):
    ...
    if not isinstance(box_list, str) and np.isnan(box_list):
        bboxes = np.asarray([[0,0,ann['width']-1,ann['height']-1]])
    else:
    ...
    ann['ann']['labels'] += bbox_df.int_label.iloc[i]
...
with open(osp.join(DATADIR, 'train_bbox_annotations_mmdet_multiclass.pkl'), 'wb') as f:
