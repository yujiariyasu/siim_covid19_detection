...
def main():
    ...
    with open(cfg.data.ann_file, 'rb') as f:
        annotations = pickle.load(f)

    train_ann, val_ann, test_ann = get_train_val_test_splits(cfg, annotations)

    cfg.data.train.update({'annotations': train_ann})
    cfg.data.val.update({'annotations': val_ann})

    datasets = [build_dataset(cfg.data.train)]
    logger.info(f'TRAIN :  N={len(datasets[0])}')
    if len(cfg.workflow) == 2:
        val_dataset = copy.deepcopy(cfg.data.val)
        val_dataset.pipeline = cfg.data.train.pipeline
        datasets.append(build_dataset(val_dataset))
        logger.info(f'VALID :  N={len(datasets[1])}')
