...
class DetBenchPredict(nn.Module):
    ...
    def forward(self, x, img_info: Optional[Dict[str, torch.Tensor]] = None, iou_threshold=0.5):
...
class DetBenchTrain(nn.Module):
    def __init__(self, model, create_labeler=True):
        ...
        #Loss will be built outside of model init and set later
        #self.loss_fn = DetectionLoss(model.config)

    def forward(self, x, target: Dict[str, torch.Tensor], iou_threshold=0.5):
        out = self.model(x)
        if len(out) == 2:
            class_out, box_out = out
            cls_out = None
        elif len(out) == 3:
            class_out, box_out, cls_out = out
        ...
        if not self.training:
            ...
            output['detections'] = _batch_detection(
                ...
                None, target['img_size'],
                max_det_per_image=self.max_det_per_image, soft_nms=self.soft_nms,
                iou_threshold=iou_threshold)
        if isinstance(cls_out, torch.Tensor):
            output['img_cls'] = cls_out
        return output
