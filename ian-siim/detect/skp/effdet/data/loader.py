...
class DetectionFastCollate:
    ...
    def __init__(
        ...
        for k in instance_keys:
            self.instance_info[k] = dict(fill=instance_fill, shape=instance_shapes[k])
    ...
    def __call__(self, batch):
        ...
        if len(batch[0]) == 3:
            # return image classification label as well
            class_labels = torch.stack([torch.from_numpy(b[2]) for b in batch])
            return img_tensor, target, class_labels
