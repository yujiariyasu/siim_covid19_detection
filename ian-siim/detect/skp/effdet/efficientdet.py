...
class EfficientDet(nn.Module):
    ...
    def __init__(self, config, pretrained_backbone=True, alternate_init=False,
        ...
        if add_classification_head: 
            ...
            if self.use_fpn_features:
                self.classifier = nn.Linear(self.config.fpn_channels * 6, 5)
            else:
                self.classifier = nn.Linear(feature_info[-1]['num_chs'], 5)
    ...
    def forward(self, x):
        x = self.backbone(x)
        if hasattr(self, 'classifier') and not self.use_fpn_features:
            features = x[-1].mean((-1,-2))
            x_cls = self.classifier(features)
        x = self.fpn(x)
        if hasattr(self, 'classifier') and self.use_fpn_features:
            features = torch.cat([feat.mean((-1,-2)) for feat in x], dim=1)
            x_cls = self.classifier(features)
        x_class = self.class_net(x)
        x_box = self.box_net(x)
        if hasattr(self, 'classifier'):
            return x_class, x_box, x_cls
