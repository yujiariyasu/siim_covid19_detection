...
def load_model(cfg, args):
    ...
    cfg.model.params.load_pretrained = None
...
def setup(cfg, args, mode='predict', return_preprocessor=False):
    ...
    if mode == 'test':
        with open(cfg.data.annotations, 'rb') as f:
            ann = pickle.load(f)
        _,_,ann = get_train_val_test_splits(cfg, ann)
    else:
        with open(args.imgfiles) as f:
            imgfiles = [line.strip() for line in f.readlines()]
        ann = [dict(filename=i, bbox=[]) for i in imgfiles]

    if args.data_dir:
        for a in ann:
            a['filename'] = osp.join(args.data_dir, a['filename'])

    if args.fast_dev_run:
        ann = ann[:args.fast_dev_run]

    if cfg.local_rank == 0:
        print(f'PREDICT : n={len(ann)}')

    cfg.data.dataset.params.return_name = True
    cfg.data.dataset.params.return_label = False
    cfg.data.dataset.params.return_imsize = True
...
def predict(cfg, args, mode='predict'):
    ...
    y_pred, names,  = [],[]
    for _ind, data in iterator:
        batch, _names, imsizes = data
        imsizes = torch.stack(imsizes, dim=1)
        batch, _ = cudaify(batch, [], device=cfg.local_rank)
        with torch.no_grad():
            output = model(batch)
        output = output[:, :, [1,0,3,2,4,5]]
        for b in range(batch.size(0)):
            scale_factor = (imsizes[b][0] / batch.size(2), imsizes[b][1] / batch.size(3))
            output[b, :, [0,2]] = output[b, :, [0,2]] * scale_factor[1]
            output[b, :, [1,3]] = output[b, :, [1,3]] * scale_factor[0]
