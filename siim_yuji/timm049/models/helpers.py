...
def load_pretrained(model, default_cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=True, progress=False, skip_attn=False):
    ...
    if skip_attn:
        print('remove attn_mask from state dict...')
        # import pdb;pdb.set_trace()
        state_dict = {k: v for k, v in state_dict.items() if 'attn_mask' not in str(k)}
    model.load_state_dict(state_dict, strict=strict)
...
def build_model_with_cfg(
    ...
    skip_attn = kwargs.pop('skip_attn', False)
    features = False
