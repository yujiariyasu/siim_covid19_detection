...
class TorchTrainer:
    ...
    def train_loop(self, loader, grad_accumulations=1, logger_interval=1, epoch=0):
        ...
        self.model.train()
        for batch_i, (images, input_ids, attention_masks, y) in enumerate(loader):
            ...
            images, input_ids, attention_masks, y = images.to(self.device), input_ids.to(self.device), attention_masks.to(self.device), y.to(self.device)

            _, _y = self.model(images, input_ids, attention_masks, y)
            if self.is_fp16:
                _y = _y.float()
        ...
        if (self.eval_metric is None) or (self.mixup) or (self.cutmix):
    ...
    def valid_loop(self, loader, grad_accumulations=1, logger_interval=1, epoch=0, tta=1):
        ...
        self.model.eval()
        with torch.no_grad():
            for images, input_ids, attention_masks, y in loader:
                images, input_ids, attention_masks, y = images.to(self.device), input_ids.to(self.device), attention_masks.to(self.device), y.to(self.device)
                ...
                if self.is_fp16:
                    preds = preds.float()
                approx.append(preds.clone().detach())
                target.append(y.clone().detach())
    ...
    def predict(self, loader, path=None, verbose=True):
        ...
        with torch.no_grad():
            for inputs in loader:
                images, input_ids, attention_masks = inputs[0].to(self.device), inputs[1].to(self.device), inputs[2].to(self.device)
                if self.is_fp16 and APEX_FLAG and (str(self.device) != 'cpu'):
                    with amp.disable_casts():
                        features, _y = self.model(images, input_ids, attention_masks)
                else:
                    features, _y = self.model(images, input_ids, attention_masks)
