...
class TorchTrainer:
    ...
    def train_loop(self, loader, grad_accumulations=1, logger_interval=1, epoch=0):
        ...
        with tqdm(loader, total=len(loader), leave=False) as pbar:
            for batch_i, (images, mask, y) in enumerate(pbar):
                images = np.array(images.view(images.size(0), 1, self.cfg.image_size, self.cfg.image_size)).repeat(3,1)
                images = np.ascontiguousarray(images)
                images = images.astype(np.float32) / 255
                images = torch.from_numpy(images)

                mask = np.array(mask.view(mask.size(0), 1, self.cfg.image_size, self.cfg.image_size))#.repeat(3,1)
                mask = np.ascontiguousarray(mask)
                mask = mask.astype(np.float32)
                mask = torch.from_numpy(mask)

                batches_done = len(loader) * self.current_epoch + batch_i
                images, mask, y = images.to(self.device), mask.to(self.device), y.to(self.device)
                # if self.train_transform is not None:
                #     images = self.train_transform(images)

                if self.cfg.mixup_aug_with_mask is not None:
                    X, y = self.cfg.mixup_aug_with_mask(images, mask, y, self.cfg.mixup_params)
                    y['y1_seg'] = F.interpolate(y['y1_seg'], size=(32, self.cfg.mask_x_resize), mode='bilinear')
                    y['y1_seg'] = y['y1_seg'].view(y['y1_seg'].size()[0], -1)
                    y['y2_seg'] = F.interpolate(y['y2_seg'], size=(32, self.cfg.mask_x_resize), mode='bilinear')
                    y['y2_seg'] = y['y2_seg'].view(y['y2_seg'].size()[0], -1)
                else:
                    mask = F.interpolate(mask, size=(32, self.cfg.mask_x_resize), mode='bilinear')
                ...
                #     if mask[i].sum == 0:
                #         _y_aux[i] = _y_aux[i]*0
                # if (self.aux_criterion is not None) and (not mix):
                #     loss_aux = self.aux_criterion(_y_aux, mask)
                #     loss = loss + loss_aux
    ...
    def valid_loop(self, loader, grad_accumulations=1, logger_interval=1, epoch=0, tta=1):
        ...
        with torch.no_grad():
            ...
            for tta_n in tqdm(range(tta)):
                ...
                for images, _, y in loader:
                    ...
                    # if self.train_transform is not None:
                    #     images = self.train_transform(images)
                    # mask = F.interpolate(mask, size=(32,48), mode='bilinear')
                    ...
                    #     images = torch.flip(images,dims=(2,))
                        # images = F.interpolate(images, scale_factor=1.33, mode='bilinear', align_corners=False)

                    if self.is_fp16 and APEX_FLAG and (str(self.device) != 'cpu'):
                        with amp.disable_casts():
                            _y, _ = self.model(images)
                    else:
                        _y, _ = self.model(images)
        ...
        target = torch.cat(target).cpu()
        # st()
        loss_total = self.cfg.cls_criterion(torch.tensor(approx), torch.tensor(target))
        # loss_total = 1
