...
class TorchTrainer:
    ...
    def train_loop(self, loader, grad_accumulations=1, logger_interval=1, epoch=0):
        ...
        for batch_i, inputs in enumerate(loader):
            ...
            if self.mixup:
                inputs = mixup(inputs)
            # if self.cutmix:
            #     inputs = cutmix(inputs)
            # if self.fmix:
            #     inputs = fmix(inputs)
            X, meta, y = inputs[0], inputs[1], inputs[2]
            X = X.to(self.device)
            meta = meta.to(self.device)
            y = y.to(self.device)

            cnn_output, _y = self.model(X, meta)
            if self.is_fp16:
                _y = _y.float()
            ...
            loss2 = self.criterion(cnn_output, y)
            loss += loss2
            if self.center_loss_criterion is not None:
                center_loss = self.center_loss_criterion(y, cnn_output)
    ...
    def valid_loop(self, loader, grad_accumulations=1, logger_interval=1, epoch=0, tta=1):
        ...
        with torch.no_grad():
            for inputs in loader:
                ...
                if self.center_loss_criterion is not None:
                    center_loss = self.center_loss_criterion(y, features)
                    loss = loss + center_loss*self.center_loss_weight
    ...
    def fit(self,
        ...
        if not self.stopper.stop():  # Not stopped by overfit detector
            if verbose:
                print(
                    f"[{self.serial}] Best score is {self.stopper.score():.{self.round_float}f}")
            load_snapshots_to_model(str(snapshot_path), self.model, self.optimizer)
